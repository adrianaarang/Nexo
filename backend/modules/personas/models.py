...
def get_all_personas(search_q: str | None = None) -> list[dict[str, Any]]:
    """Consulta y retorna todas las personas activas (no eliminadas)"""
    with get_cursor() as cursor:
        query = "SELECT * FROM personas WHERE is_deleted = 0"
        params: list[Any] = []
...
def get_persona_by_id(person_id: int) -> dict[str, Any] | None:
    """Busca y devuelve el registro de una persona activa por su ID"""
    with get_cursor() as cursor:
        cursor.execute(
            "SELECT * FROM personas WHERE id = ? AND is_deleted = 0",
            (person_id,),
        )
        return _row_to_dict(cursor.fetchone())

def create_persona(data: dict[str, Any]) -> dict[str, Any] | None:
    """Inserta una nueva persona en la base de datos y retorna el registro creado"""
    with get_cursor() as cursor:
        cursor.execute(
            """
            ...
            (
                ...
                data.get("estado", PersonStatus.MISSING.value),
...
def mark_person_safe(person_id: int) -> dict[str, Any] | None:
    """Marca como segura una persona ya registrada e incrementa su versión de concurrencia"""
    ...
    with get_cursor() as cursor:
        cursor.execute(
            """
            UPDATE personas
            SET estado = ?, version = version + 1, updated_at = CURRENT_TIMESTAMP
